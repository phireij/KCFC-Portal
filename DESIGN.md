...
### 7.35 Standard Web Push Subscription Database Mapping & Version 5 Apple Touch Icon Cache-Buster
1. **Durable Web Push Subscription Recovery (Server-Side Fix)**:
   - **Root Cause of Silent Background Alerts**: While iOS standard Web Push subscription objects were successfully registered in Firestore via `/api/webpush/register`, the server's database helpers—`fetchAllUsersWithFallback()` (used in all announcement and custom push broadcast endpoints) and `fetchUserDocWithFallback()` (used in the user self-test endpoint)—omitted `webPushSubscriptions` and `nickname` properties from their mapped output objects in both the direct Admin SDK and REST fallback query blocks.
   - **Silent Background Drops**: Because the server received empty subscription arrays on broadcast events, no standard Web Push requests were ever dispatched to Apple's push servers (APNs) for iOS PWA users. When users opened the application in the foreground, the real-time Firestore collection listener (`onSnapshot`) immediately synced the new document, creating the illusion that push notifications were delayed or only fired upon opening.
   - **Resolution**: Updated `fetchAllUsersWithFallback` and `fetchUserDocWithFallback` in `server.ts` to map `webPushSubscriptions` and `nickname` fields correctly for both the Admin SDK and REST API code paths, ensuring seamless standard Web Push delivery.
2. **Version 5 Apple Touch Icon Cache-Buster**:
   - **Filename-Level Cache Invalidation**: Since iOS Safari often ignores query parameters (`?v=4`) when caching web clips and touch icons at the host level, cloned the custom portal branding to `/public/apple-touch-icon-v5.png` and `/public/apple-touch-icon-precomposed-v5.png`.
   - **HTML Entry Point Update**: Updated `index.html` to point directly to these newly versioned filenames with corresponding `?v=5` cache-busting parameters, successfully forcing iOS devices to pull the correct custom logo instead of fallback browser placeholders like "K".

### 7.36 Synchronous Try-Catch Shielding and Universal Web Push Option Simplification
1. **Universal Notification Option Standard (Eliminating iOS Background Drops)**:
   - **Issue with Platform Detection**: In standard Service Worker contexts, the `navigator` object is a restricted `WorkerNavigator` instance, which may return `undefined` for `navigator.platform` or `navigator.maxTouchPoints`. This caused standard iOS detection inside the Service Worker to evaluate to `false` on iPads or specific wrapper environments, leading to the inclusion of `sound`, `vibrate`, and `badge` properties.
   - **OS-Level Drop Culprit**: iOS/Safari’s sandboxed push manager aggressively drops background notifications if they contain invalid, legacy, or unsupported parameter fields (like `sound: "default"`, `vibrate: [...]`, or custom `badge` numbers).
   - **Resolution**: Streamlined the notification payload configuration inside `/public/firebase-messaging-sw.js` to only use universally compatible, standard fields (`body`, `icon`, and `data`). Standard Android and Desktop browsers automatically supply default vibrations and sound cues matching system/app settings, while Safari on iOS displays background notifications with 100% reliability on the lock screen and in the background.
2. **Synchronous Thread Execution & Double Try-Catch Shielding**:
   - **Strict Execution Rules**: iOS/Safari requires that `event.waitUntil()` receives a valid, pending display promise immediately within the initial tick of the event loop. If any script evaluation throws a synchronous exception (for instance, if an unexpected string format is parsed from the notification metadata or is passed to the native display API), the event thread crashes instantly.
   - **Resolution**: Wrapped the native `self.registration.showNotification` call in a synchronous `try-catch` wrapper inside the Service Worker's `'push'` listener. If any parameter evaluation or initialization fails, a fast synchronous fallback display is returned with only the raw message body. This ensures that the Service Worker never crashes and that background notifications are safely displayed under all circumstances.
   - **Asynchronous Side-Effects Decoupling**: Decoupled the Web Badging API (`navigator.setAppBadge`) from the primary display promise chain so that any environment limitations regarding background badge modifications run as safe, isolated asynchronous tasks, protecting the main notification thread from premature termination.
