...
The latest validated configuration checkpoint before this runbook was created is `b4baee6cb1611cf67eacf2b570a7c3db29196549`, with `KCFC Redevelopment CI` run **#1088** / id `34578756178` completing successfully with all **54** validation/build/security steps green.
...
The browser public key and server public key must match. The private key must never use a `VITE_*` name or otherwise enter the browser bundle.
...
Expected application-level result: HTTP success with `status: "ok"` and a timestamp.
